...
Whining is a care-soliciting vocalization. Puppies produce it from the first days of life to bring the dam back to the nest, and unlike most neonatal behaviors it doesn't disappear at maturity — it carries into adult life as a general-purpose distance-reducing signal. That is its one consistent property across all six categories: a whining dog is attempting to close a gap between a current state and a desired one, social, physical, or internal.

Acoustically it is the growl's opposite: low harsh vocalizations broadly signal threat, high tonal ones signal need. That pattern holds across many mammals, which is why a whine reads as urgent to human ears without any training.
...
**Context.** Appears with a specific person, in a specific location, at times that historically produced a payoff. Almost never happens when the dog is genuinely alone, because there's no audience.
...
**"Small breeds just whine more."** Some individuals are more vocal at baseline, and baseline is the point — what matters is a change from *this* dog's normal, never a breed average.
